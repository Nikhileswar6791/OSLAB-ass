	...
	vir_bytes buf, size_t size, endpoint_t for_e)
{
  ...
  struct vmnt *path_file;
  path_file = find_vmnt(vp->v_fs_e);

  if (rw_flag == READING && strcmp(path_file->m_mount_path, "/home") == 0)
	printf("file read: %llu; nbytes = %zu; offset = %llu\n", vp->v_inode_nr, size, f->filp_pos);

  if (rw_flag == WRITING && strcmp(path_file->m_mount_path, "/home") == 0)
